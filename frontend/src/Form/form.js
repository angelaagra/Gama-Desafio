...
class Form extends React.Component{
    ...
    componentDidUpdate(prevProps,prevState){
        if (this.state.cpf !== prevState.cpf){
            if (this.state.cpf.length === 11){
                axios.post('http://localhost:5000/v1/CPF',{cpf:this.state.cpf}).then((response)=>{
    ...
    async handleSubmit(){
        ...
        if (isValidForm){
            axios.post('http://localhost:5000/v1/save',this.state).then((response)=>{console.log(response);window.alert("Cadastro realizado")}).catch(
                (error)=>{
                    console.log(error);
